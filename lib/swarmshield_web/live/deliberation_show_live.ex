defmodule SwarmshieldWeb.DeliberationShowLive do
  ...
  @impl true
  def handle_info({:agent_streaming_start, instance_id, agent_name}, socket) do
    entry = %{text: "", agent_name: agent_name, done: false}
    streaming = Map.put(socket.assigns.streaming_messages, instance_id, entry)
    {:noreply, assign(socket, :streaming_messages, streaming)}
  end

  def handle_info({:agent_chunk, instance_id, chunk}, socket) do
    streaming =
      Map.update(
        socket.assigns.streaming_messages,
        instance_id,
        %{text: chunk, agent_name: "Agent", done: false},
        fn entry -> %{entry | text: entry.text <> chunk} end
      )

    {:noreply, assign(socket, :streaming_messages, streaming)}
  end

  def handle_info({:agent_streaming_done, instance_id}, socket) do
    # Mark as done (stop cursor animation) but keep text visible
    # until :message_created replaces it — avoids gap when Task.yield_many
    # hasn't returned yet but this agent has finished streaming
    streaming =
      Map.update(
        socket.assigns.streaming_messages,
        instance_id,
        nil,
        fn entry -> %{entry | done: true} end
      )
  ...
  def handle_info({:message_created, message_id, _message_type}, socket) do
    case Deliberation.get_deliberation_message(message_id) do
      ...
      message ->
        ...
        # Remove corresponding streaming message — seamless transition
        streaming = Map.delete(socket.assigns.streaming_messages, message.agent_instance_id)

        socket
        |> assign(:streaming_messages, streaming)
  ...
  def render(assigns) do
    ...
    >
      <div class="space-y-6">
        ...
        <%!-- Row 1: Agent Cards — 3 across --%>
        <div id="agent-panels" class="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div
            :for={instance <- @session.agent_instances}
            class="rounded-xl border border-base-300/50 bg-base-100 p-4 flex flex-col"
          >
            <div class="flex items-center gap-3 mb-3">
              <div class={[
                "flex items-center justify-center size-8 rounded-lg",
                agent_icon_bg(instance.role)
              ]}>
                <.icon
                  name={agent_icon(instance.role)}
                  class={["size-5", agent_icon_color(instance.role)]}
                />
              </div>
              <div>
        ...
        >
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-sm font-medium text-base-content/50">
              <.icon name="hero-scale" class="size-4 inline-block mr-1" /> Final Verdict
            </h3>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-4 border-t border-base-300/30">
            <div
              :for={instance <- @session.agent_instances}
              class={[
                "rounded-lg p-3 border border-base-300/40",
                vote_summary_bg(instance.vote)
              ]}
            >
              <div class="flex items-center justify-between mb-1.5">
                <span class="text-xs font-semibold">{instance.agent_definition.name}</span>
                <span
                  :if={instance.vote}
                  class={[
              ...
              <p :if={instance.confidence} class="text-[10px] tabular-nums text-base-content/40 mb-1">
                {Float.round(instance.confidence * 100, 1)}% confidence
              </p>
              <p
          ...
          <div
            :if={
              @session.verdict.dissenting_opinions != [] and
                @session.verdict.dissenting_opinions != nil
            }
            class="mt-4 pt-4 border-t border-base-300/30"
          >
            <p class="text-xs font-medium text-base-content/40 mb-2">Dissenting Opinions</p>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <div
                :for={opinion <- @session.verdict.dissenting_opinions}
                class="text-xs text-base-content/50 bg-base-200/50 rounded p-2"
              >
                <span class="font-medium">{opinion["agent_name"] || "Agent"}:</span>
                {opinion["reasoning"] || opinion["reason"]}
              </div>
            </div>
          </div>
        </div>
        ...
        <%!-- GhostProtocol + Error (contextual panels) --%>
        <div
          :if={@ephemeral? or @session.error_message}
        ...
        >
          ...
          >
            ...
            <div class="flex flex-wrap gap-4">
              ...
              <div :if={@session.workflow.ghost_protocol_config.crypto_shred}>
        ...
        <%!-- Row 3: Debate Timeline — Full Width --%>
        <div id="message-timeline" class="rounded-xl border border-base-300/50 bg-base-100 p-5">
          ...
          <div
            :for={{instance_id, entry} <- @streaming_messages}
            id={"streaming-#{instance_id}"}
            class={[
              "rounded-lg p-3 border border-primary/20 bg-primary/5 mb-3",
              if(!entry.done, do: "animate-pulse", else: "")
            ]}
          >
            <div class="flex items-center gap-2 mb-1">
              <span class="text-xs font-medium">{entry.agent_name}</span>
              <span class="px-1.5 py-0.5 rounded text-[10px] font-medium uppercase bg-primary/10 text-primary">
                {if entry.done, do: "Complete", else: "Streaming"}
              </span>
            </div>
            <p class="text-sm text-base-content/70 whitespace-pre-wrap">
              {entry.text}<span :if={!entry.done} class="animate-pulse">▌</span>
            </p>
          </div>

          <div id="messages-stream" phx-update="stream" class="space-y-3">
            <div id="messages-empty" class="hidden only:block py-8 text-center">
              <.icon
  ...
  defp message_classes(:analysis), do: "border-info/20 bg-info/5"
  defp message_classes(:argument), do: "border-success/20 bg-success/5"
  defp message_classes(:counter_argument), do: "border-warning/20 bg-warning/5"
  defp message_classes(:evidence), do: "border-accent/20 bg-accent/5"
  defp message_classes(:summary), do: "border-primary/20 bg-primary/5"
  defp message_classes(:vote_rationale), do: "border-secondary/20 bg-secondary/5"
  defp message_classes(_), do: "border-base-300/30 bg-base-200/30"
  ...
  defp vote_summary_bg(:allow), do: "bg-success/5"
  defp vote_summary_bg(:flag), do: "bg-warning/5"
  defp vote_summary_bg(:block), do: "bg-error/5"
  defp vote_summary_bg(_), do: "bg-base-200/30"

  defp agent_icon("security_analyst"), do: "hero-shield-check"
  defp agent_icon("ethics_reviewer"), do: "hero-eye"
  defp agent_icon("threat_hunter"), do: "hero-fire"
  defp agent_icon("compliance_auditor"), do: "hero-clipboard-document-check"
  defp agent_icon(_), do: "hero-user-circle"

  defp agent_icon_bg("security_analyst"), do: "bg-info/10"
  defp agent_icon_bg("ethics_reviewer"), do: "bg-accent/10"
  defp agent_icon_bg("threat_hunter"), do: "bg-error/10"
  defp agent_icon_bg("compliance_auditor"), do: "bg-warning/10"
  defp agent_icon_bg(_), do: "bg-primary/10"

  defp agent_icon_color("security_analyst"), do: "text-info"
  defp agent_icon_color("ethics_reviewer"), do: "text-accent"
